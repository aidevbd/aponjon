...
export function ContactFilters({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {/* Search */}
      <div className="relative min-w-0 flex-1 basis-full sm:basis-auto sm:flex-1">
      ...
      <FilterDropdown
        ...
        activeLabel={
          activeCategory ? `${activeCategory.icon} ${activeCategory.value}` : undefined
        }
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className={`flex items-center gap-1.5 rounded-sm border px-2.5 py-2 text-[12px] transition-colors ${
          active
      ...
      {open && (
        <div className="absolute right-0 z-30 mt-1.5 min-w-[180px] rounded-sm border border-[hsl(var(--heirloom-line))] bg-[hsl(var(--heirloom-paper))] shadow-[0_12px_32px_-12px_hsl(var(--heirloom-ink)/0.25)]">
