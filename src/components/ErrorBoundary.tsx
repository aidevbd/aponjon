        ...
        >
          <article className="heirloom-page relative w-full max-w-xl overflow-hidden rounded-sm border p-6 sm:p-10">
            ...
            <div className="relative flex flex-col items-center text-center">
              <p className="mt-6 font-display text-5xl leading-none tracking-tight text-[hsl(var(--heirloom-gold-deep))] sm:text-6xl">
                ও মা!
              </p>

              <div
                aria-hidden
                className="mt-5 h-px w-24 bg-gradient-to-r from-transparent via-[hsl(var(--heirloom-gold))] to-transparent"
              />

              <h1 className="mt-5 font-display text-2xl leading-[1.2] tracking-tight text-[hsl(var(--heirloom-ink))] sm:text-3xl">
                একটু অসুবিধা হয়েছে
              </h1>

              <p className="mt-4 max-w-md text-[15px] leading-[1.6] text-[hsl(var(--heirloom-ink-soft))] sm:text-base">
                অপ্রত্যাশিত কিছু একটা ঘটে গেছে। চিন্তার কিছু নেই — আপনার তথ্য নিরাপদ আছে। নিচের যেকোনো একটি করে দেখুন।
              </p>

              {import.meta.env.DEV && this.state.error && (
                <pre className="mt-5 w-full max-w-md overflow-x-auto rounded-sm border border-[hsl(var(--heirloom-line))] bg-[hsl(var(--heirloom-cream)/0.5)] p-3 text-left text-[11px] text-[hsl(var(--heirloom-ink-mute))]">
              ...
              <div className="mt-8 flex w-full max-w-[420px] flex-col gap-3">
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center rounded-sm border border-[hsl(var(--heirloom-line))] bg-transparent px-5 py-3.5 text-[14px] font-medium text-[hsl(var(--heirloom-ink))] transition-colors hover:bg-[hsl(var(--heirloom-cream)/0.5)]"
                >
                  পেজটি রিলোড করুন
                </button>
                <button
                  onClick={this.handleHome}
                  className="flex w-full items-center justify-center rounded-sm px-5 py-2 text-[13px] text-[hsl(var(--heirloom-ink-soft))] transition-colors hover:text-[hsl(var(--heirloom-gold-deep))]"
                >
                  হোমপেজে ফিরে যাই
                </button>
              </div>
